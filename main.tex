...
chain, \sout{%
    and in particular, we can define the tunneling rate, \(\delta Q\) as%
}
...
\end{equation}
%
\sout{where the difference is between subsequent states in the chain.}
%
\sout{This quantity is analogous to the lag-one autocorrelation in the
topological charge.}
%
...
\begin{figure}[htpb]
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \caption{\label{subfig:loss}Training loss
        \(\beta = 7\). Note \(\delta Q \simeq 0\)}
    \end{subfigure}
    \begin{subfigure}[b]{\textwidth}
...
reusing the same data from the \(8\times8\) run (the Trained data is
correct, however). I will update this soon.}
